...
export class PendidikanAggregator {
  ...
  async aggregateByDosen(
    ...
    // Query yang ambil semua nilai string langsung dari database
  ...
  private buildStructuredResult(
    ...
    // Build structure secara dinamis berdasarkan data yang ada
    for (const row of rawData) {
      const { kategori, detail, total, count, pending, approved, rejected } = row;
      
      // Initialize kategori jika belum ada
      if (!result[kategori]) {
        result[kategori] = {
          total: 0,
          count: 0,
      ...
      // Update detail level secara dinamis
