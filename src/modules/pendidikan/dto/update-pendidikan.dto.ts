...
export const pendidikanFormalUpdateSchema = z.object({
  ...
  prodi: z.string().trim().min(1, { message: 'Prodi wajib diisi' }).optional(),
  fakultas: z.string().trim().min(1, { message: 'Fakultas wajib diisi' }).optional(),
  perguruanTinggi: z.string().trim().min(1, { message: 'Perguruan Tinggi wajib diisi' }).optional(),
...
export const pendidikanDiklatUpdateSchema = z.object({
  ...
  namaDiklat: z.string().trim().min(1, { message: 'Nama Diklat wajib diisi' }).optional(),
  jenisDiklat: z.string().trim().min(1, { message: 'Jenis Diklat wajib diisi' }).optional(),
  penyelenggara: z.string().trim().min(1, { message: 'Penyelenggara wajib diisi' }).optional(),
  peran: z.string().trim().min(1, { message: 'Peran wajib diisi' }).optional(),
  tingkatan: z.string().trim().min(1, { message: 'Tingkatan wajib diisi' }).optional(),
  jumlahJam: z.coerce.number().positive({ message: 'Jumlah jam harus lebih dari 0' }).optional(),
  noSertifikat: z.string().trim().min(1, { message: 'No Sertifikat wajib diisi' }).optional(),
  tglSertifikat: z.coerce.date({ message: 'Tanggal sertifikat wajib diisi' }).optional(),
  tempat: z.string().trim().min(1, { message: 'Tempat wajib diisi' }).optional(),
  tglMulai: z.coerce.date({ message: 'Tanggal mulai wajib diisi' }).optional().refine(
    (d) => d == null || !isNaN(d.getTime()),
    { message: 'Tanggal mulai tidak valid.' }
  ),
  tglSelesai: z.coerce.date({ message: 'Tanggal selesai wajib diisi' }).optional().refine(
    (d) => d == null || !isNaN(d.getTime()),
  ...
  (data) => {
    if (data.tglMulai && data.tglSelesai) {
      return data.tglSelesai >= data.tglMulai;
    }
    return true;
  },
  {
