      ...
      WITH pelaksanaan_summary AS (
        SELECT 
          ...
          ${includeDetail 
            ? Prisma.sql`
              CASE 
                WHEN p."kategori" = 'BIMBINGAN_TUGAS_AKHIR' THEN bta."jenis"
                WHEN p."kategori" = 'PENGUJI_UJIAN_AKHIR' THEN pu."peran"
                WHEN p."kategori" = 'BAHAN_PENGAJARAN' THEN bp."jenis"
                WHEN p."kategori" = 'JABATAN_STRUKTURAL' THEN js."namaJabatan"
                WHEN p."kategori" = 'BIMBING_DOSEN' THEN bd."bidangAhli"
                WHEN p."kategori" = 'DATA_SERING_PENCAKOKAN' THEN dp."jenis"
                WHEN p."kategori" = 'PENGEMBANGAN_DIRI' THEN pd."namaKegiatan"
                ELSE NULL
              END as detail
            `
